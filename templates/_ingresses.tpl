...
{{- define "helmLibraryChart.ingresses" -}}
  ...
  {{- range $name, $ingress := .Values.ingress }}
    {{- if $ingress.enabled -}}
      ...
      {{/* set defaults */}}
      {{- if and (not $ingressValues.nameOverride) (ne $name (include "helmLibraryChart.ingress.primary" $)) -}}
        {{- $_ := set $ingressValues "nameOverride" $name -}}
      {{- end -}}

      {{- $_ := set $ "ObjectValues" (dict "ingress" $ingressValues) -}}
      {{- include "helmLibraryChart.ingress" $ }}
    {{- end }}
  {{- end }}
{{- end }}

{{/* Return the name of the primary ingress object */}}
{{- define "helmLibraryChart.ingress.primary" -}}
