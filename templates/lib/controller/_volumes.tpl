...
*/}}
{{- define "helmLibraryChart.controller.volumes" -}}
    ...
    {{- else -}}
      ...
      {{- if $persistence.nameOverride -}}
        {{- if not (eq $persistence.nameOverride "-") -}}
          {{- $pvcName = (printf "%s-%s" (include "helmLibraryChart.names.fullName" $) $persistence.nameOverride) -}}
        {{- end -}}
      {{- else -}}
        {{- $pvcName = (printf "%s-%s" (include "helmLibraryChart.names.fullName" $) $index) -}}
      {{- end -}}
    {{- end }}
